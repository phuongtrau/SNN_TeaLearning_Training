...
for i in range(len(train_data)):
    ...
    if train_label[i] == 1 or train_label[i]==3:
...
for i in range(len(test_data)):
    ...
    if test_label[i] == 1 or test_label[i] == 3:
...
checkpoint_filepath = './ckpt_fashion/2_class_fashion'

import keras 
model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
    filepath=checkpoint_filepath + '-epoch-{epoch}',
    save_weights_only=True,)
model.load_weights('./ckpt_fashion/2_class_fashion-epoch-20')
