...
for i in range(len(train_data_right.samples)):
    mask = np.ones_like(train_data_right.samples[i])
    
    train_data_right.samples[i] = cv2.equalizeHist(train_data_right.samples[i])

    e_1 = np.array(train_data_right.samples[i]>=mask*31).astype(float)
    e_2 = np.array(train_data_right.samples[i]>=mask*63).astype(float)
    e_3 = np.array(train_data_right.samples[i]>=mask*95).astype(float)
    e_4 = np.array(train_data_right.samples[i]>=mask*127).astype(float)
    e_5 = np.array(train_data_right.samples[i]>=mask*159).astype(float)
    e_6 = np.array(train_data_right.samples[i]>=mask*191).astype(float)
    e_7 = np.array(train_data_right.samples[i]>=mask*223).astype(float)
    # print(e_1[:,:,np.newaxis].shape)
    x_train.append(np.concatenate((e_1[:,:,np.newaxis],e_2[:,:,np.newaxis],e_3[:,:,np.newaxis],\
                                e_4[:,:,np.newaxis],e_5[:,:,np.newaxis],e_6[:,:,np.newaxis],\
                                e_7[:,:,np.newaxis]),axis=2,dtype=np.float64))
...
for i in range(len(test_data_right.samples)):
    # test_data.samples[i] = cv2.equalizeHist(test_data.samples[i])
    x_test_3_class.append(cv2.equalizeHist(test_data_right.samples[i]))
...
for i in range(len(train_data_right.samples)):
    # test_data.samples[i] = cv2.equalizeHist(test_data.samples[i])
    x_train_3_class.append(cv2.equalizeHist(train_data_right.samples[i]))
...
y_train = to_categorical(train_data_right.labels, 17)
