...
for sub in ls_train_full:
  ...
  if not os.path.exists("mem_files/right_21_cores_mem_nocrop/{}".format(sub)):
    os.makedirs("mem_files/right_21_cores_mem_nocrop/{}".format(sub))

  cores_sim = create_cores(model, 16*9+5 , neuron_reset_type=0,num_classes=4) 

  write_cores(cores_sim,max_xy=(1,16*9+5),output_path="mem_files/right_21_cores_mem_nocrop/{}".format(sub))
  fold_no = fold_no + 1
