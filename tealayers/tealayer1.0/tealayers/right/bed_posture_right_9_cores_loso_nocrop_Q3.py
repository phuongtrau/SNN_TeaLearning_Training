...
for sub in ls_train_full:
  ...
  for i in range(len(test_data.samples)):
      ...
      e_1 = np.array(test_data.samples[i]>=mask*63).astype(float)
      e_2 = np.array(test_data.samples[i]>=mask*127).astype(float)
      e_3 = np.array(test_data.samples[i]>=mask*190).astype(float)

      # print(e_1[:,:,np.newaxis].shape)
      x_test.append(np.concatenate((e_1[:,:,np.newaxis],e_2[:,:,np.newaxis],e_3[:,:,np.newaxis]),axis=2))
